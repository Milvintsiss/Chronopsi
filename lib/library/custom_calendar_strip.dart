...
class CustomCalendarStrip extends StatefulWidget {
  ...
  State<CustomCalendarStrip> createState() =>
      CustomCalendarStripState(selectedDate, startDate, endDate);
}

class CustomCalendarStripState extends State<CustomCalendarStrip>
  ...
  bool isOnEndingWeek = false,
      isOnStartingWeek = false;
  ...
  runPresetsAndExceptions(selectedDate, startDate, endDate) {
    ...
    } else if (selectedDate != null && (isDateBefore(selectedDate, startDate) ||
  ...
  void initState() {
    super.initState();
    rowStartingDate =
    rowStartingDate != null ? rowStartingDate : currentDate.subtract(
  ...
  String getMonthLabel() {
    ...
    if (startingDayObj.month == endingDayObj.month) {
      ...
      var startingDayYear = "${startingDayObj.year == endingDayObj.year
          ? ""
          : startingDayObj.year}";
      label =
      "${getMonthName(startingDayObj)} $startingDayYear / ${getMonthName(
          endingDayObj)} ${endingDayObj.year}";
    }
    return label;
  }
  ...
  Map<String, bool> calculateDateRange(mode) {
    if (doesDateRangeExists) {
      ...
      bool isStartingWeekOnRange = isDateAfter(
          widget.startDate, weekStartingDate);
  ...
  monthLabelWidget(monthLabel) {
    ...
    return Container(child: Text(monthLabel, style: monthLabelStyle),
  ...
  Widget dateTileBuilder(DateTime date, DateTime selectedDate, int rowIndex) {
          ...
          child: InkWell(
            ...
            child: Container(
              child: widget.dateTileBuilder(
                  date, selectedDate, rowIndex, dayName, isDateMarked(date),
                  isDateOutOfRange),
            ),
          ),
        ),
    ...
    var normalStyle = TextStyle(fontSize: 17,
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: !isSelectedDate ? Colors.transparent : widget
                  .selectedColor,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: buildDateRow(),
      decoration: widget.containerDecoration != null ? widget
          .containerDecoration : BoxDecoration(),
    );
  }

  @override
  void didUpdateWidget(CustomCalendarStrip oldWidget) {
    super.didUpdateWidget(oldWidget);
    print("Old day: ${oldWidget.selectedDate.day}");
    print("Current day: ${widget.selectedDate.day}");
    print("Selected day: ${selectedDate.day}");
    print("---------------------");
    if (!isTap && oldWidget.selectedDate != widget.selectedDate) {
    ...
    // if(!isTap && oldWidget.selectedDate != selectedDate){
    //   print("Old day: ${oldWidget.selectedDate.day}");
    //   print("Current day: ${selectedDate.day}");
